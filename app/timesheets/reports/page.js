...
const TimesheetReports = () => {
  ...
  const filteredTimesheets = timesheets.filter((entry) => {
    ...
    return (
      ...
      (districtFilter === '' || entry.district.toLowerCase() === districtFilter.toLowerCase()) &&
      (periodFilter === '' || entry.period.toLowerCase().includes(periodFilter.toLowerCase()))
    );
  });
  ...
  const handleDownload = async (entry) => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/timesheets/${entry.id}/`, {
        method: "GET",
        headers: {
      ...
      window.setTimeout(() => {
        ...
        if (downloadelement) {
          ...
          html2pdf().set(opt).from(downloadelement).save().then(() => {
  ...
  const paginatedTimesheets = filteredTimesheets.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
  return (
    <div>
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        <DialogTitle>Export Timesheet Report</DialogTitle>
        <DialogContent>
          <TimesheetReport handleFilterChange={handleFilterChange} handleExport={handleExport} filters={filters} districts={districts} />
        </DialogContent>
      </Dialog>
      <Typography variant="h5" gutterBottom sx={{ fontWeight: "700", marginTop: "10px" }}>
        Timesheet Report
      </Typography>
     
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <Button variant="contained" onClick={handleShowExport}> Export Reports </Button>
  ...
  >
    <MenuItem value="">All</MenuItem>
    {[...new Set(timesheets.map((timesheet) => timesheet.period))].map((period, index) => (
